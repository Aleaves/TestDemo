...
public class WheelSurfView extends RelativeLayout {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        LinearLayout mViewContainer = new LinearLayout(context);
        mViewContainer.setBackgroundResource(R.mipmap.icon_start_reward);
        mViewContainer.setOrientation(LinearLayout.VERTICAL);
        mViewContainer.setGravity(Gravity.CENTER);
        LayoutParams mCenterParams = new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
        mCenterParams.addRule(RelativeLayout.CENTER_IN_PARENT);
        mViewContainer.setLayoutParams(mCenterParams);

        ImageView mRewardView = new ImageView(context);
        mRewardView.setImageResource(R.mipmap.icon_has_reward_text);

        TextView mRewardTextView = new TextView(context);
        mRewardTextView.setGravity(Gravity.CENTER);
        mRewardTextView.setPadding(0, 10, 0, 0);

        mRewardTextView.setText("1次机会");

        mRewardTextView.setTextColor(Color.WHITE);
        mRewardTextView.setTextSize(9);

        mViewContainer.addView(mRewardView);
        mViewContainer.addView(mRewardTextView);
